...
describe("T87 — ce qu'on vient de cuisiner refroidit au frigo", () => {
  // LE COMMENTAIRE DISAIT VRAI, LE CODE ÉCRIVAIT FAUX, ET PERSONNE NE LES
  // COMPARAIT. `journaliserCuisson` annonçait depuis T26 « `location: "frigo"`
  // et pas l'espace de destination » — mais `LotStock` n'avait qu'un champ, et
  // c'est la destination qui y allait. Mesuré : 81 des 126 emits du corpus
  // déclarent `espace: congelo`, donc près des deux tiers de ce qu'on cuisine
  // se rangeait au congélateur d'un clic sur « fait ».

  test("la bolognaise sort au frigo, même si sa place est au congélo", async () => {
    await journaliserCuisson(base, {
      jour: "2026-09-01", repas: "diner", plat: bolognaise, parts: 4,
    });
    const [l] = await base.stock.toArray();
    // Les deux champs, et ils ne disent pas la même chose : `espace` est la
    // DESTINATION (le budget du congélateur la compte), `location` est où le
    // bocal se trouve ce soir.
    expect(l!.espace).toBe("congelo");
    expect(l!.location).toBe("frigo");
  });

  test("le lot vieillit donc sur l'horloge du frigo, pas sur les 90 jours", async () => {
    // C'EST LA CONSÉQUENCE QUI COÛTAIT LE PLUS CHER. Un lot cru congelé court
    // sur le forfait du congélateur ; la médiane des `gardeFrigo` de ces 81
    // emits est de 3 jours. Un reste de trois jours qui en vit quatre-vingt-dix
    // ne remonte jamais comme urgent, donc ne se mange jamais à temps.
    await journaliserCuisson(base, {
      jour: "2026-09-01", repas: "diner", plat: bolognaise, parts: 4,
    });
    const [l] = await base.stock.toArray();
    const depot = new Depot(
      { frigo: 4, congelo: 90, parType: new Map([["sauce-bolognaise", 3]]) },
      [auModele(l!)],
    );
    const vie = depot.vie(depot.lignes[0]!, new Date("2026-09-05T12:00:00"));
    expect(vie?.source).toBe("type");
    expect(vie?.fenetre).toBe(3);
    expect(vie?.depasse).toBe(true);
  });

  test("un relevé du congélo ne balaye plus ce qui refroidit au frigo", async () => {
    // Le relevé interrogeait l'index `espace`. Un bocal cuisiné le matin y
    // figurait comme congelé : relever le congélateur le soir l'aurait effacé
    // sans jamais l'avoir vu.
    await journaliserCuisson(base, {
      jour: "2026-09-01", repas: "diner", plat: bolognaise, parts: 4,
    });
    await releverDepot(base, "congelo", []);
...
describe("T32 — le relevé du dépôt referme le trou du hors-plan", () => {
  test("ce qui n'est pas gardé n'est plus là", async () => {
    const a = await base.stock.add(lot({ type: "ratatouille", espace: "congelo", location: "congelo" }));
    const b = await base.stock.add(lot({ type: "fantome", espace: "congelo", location: "congelo" }));
    await base.stock.add(lot({ type: "reste-frigo", espace: "frigo", location: "frigo" }));
